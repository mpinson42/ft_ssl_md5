...
{
	return ((((int)((unsigned)x >> n)) | (x << (32 - n))));
}
...
int sha256(uint8_t *init_msg, size_t len, t_gen *g)
{
    ...
    while(g->offset<g->new_len) {
    	...
    	while (g->i < 64)
    	{
    		uint32_t s0   = rigthrotat(g->w[g->i-15], 7) ^ rigthrotat(g->w[g->i-15], 18) ^ (g->w[g->i-15] >> 3);
            uint32_t s1   = rigthrotat(g->w[g->i-2], 17) ^ rigthrotat(g->w[g->i-2], 19) ^ (g->w[g->i-2] >> 10);

            g->w[g->i] = g->w[g->i-16] + s0 + g->w[g->i-7] + s1;
    		g->i++;
    	}
	    ...
	    while(g->i < 64)
	    {
	    	uint32_t S1 = rigthrotat(g->e, 6) ^ rigthrotat(g->e, 11) ^ rigthrotat(g->e, 25);
            uint32_t ch = (g->e & g->f) ^ ((~g->e) & g->g);

            uint32_t temp1 = g->h + S1 + ch + k2[g->i] + g->w[g->i];

            uint32_t S0 = rigthrotat(g->a, 2) ^ rigthrotat(g->a, 13) ^ rigthrotat(g->a, 22);
            uint32_t maj = (g->a & g->b) ^ (g->a & g->c) ^ (g->b & g->c);
